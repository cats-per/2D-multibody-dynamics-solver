...
if sily
    for i = 1:numel(sila)
        id = sila(i).cialo;
        F(3*id-2:3*id) = F(3*id-2:3*id) + [eye(2); (Om * Rot(q(3*id)) * sila(i).s)'] * sila(i).wekt;
    end
end
...
if tlumiki
    for i = 1:numel(tlumik)
        ...
        ri = dq(3*idi-2:3*idi-1);
        rj = dq(3*idj-2:3*idj-1);
        fii = q(3*idi);
        fij = q(3*idj);
        dfii = dq(3*idi);
        dfij = dq(3*idj);
        ...
        dd = (Rot(fii) * postep(tlumik(i).para).u)' * (vb - va);
...
if sprezyny
    for i = 1:numel(sprezyna)
        ...
        ri = q(3*idi-2:3*idi-1);
        rj = q(3*idj-2:3*idj-1);
        fii = q(3*idi);
        fij = q(3*idj);
        ...
        d = norm(sab);
        F(3*idi-2:3*idi-1) = F(3*idi-2:3*idi-1) + Rot(fii) * postep(sprezyna(i).para).u * (d - sprezyna(i).d0) * sprezyna(i).k;
        F(3*idj-2:3*idj-1) = F(3*idj-2:3*idj-1) - Rot(fii) * postep(sprezyna(i).para).u * (d - sprezyna(i).d0) * sprezyna(i).k;
    end
end
